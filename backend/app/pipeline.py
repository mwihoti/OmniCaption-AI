...
def fireworks_chat(
    ...
    j = resp.json()
    # Best-effort extraction of textual content from common response shapes
...
def groq_chat(
    ...
    """Call Groq chat completion API. Prefer Fireworks if configured; fall back to Groq.
    If no external model is available, return a safe placeholder string so the pipeline can continue."""
    ...
    if FIREWORKS_API_KEY:
        try:
            ...
            # Fireworks failed; fall back to Groq
            pass
    ...
    if GROQ_API_KEY:
        ...
        resp.raise_for_status()
        # Best-effort: handle chat message shape
        j = resp.json()
        if isinstance(j, dict):
    ...
    # No external model available — return safe placeholder text
    return "This is a mock generated text for testing."
    ...
    if FIREWORKS_API_KEY:
        try:
            ...
            if isinstance(j, dict):
                ...
                segments = j.get("segments") or j.get("segments", [])
                return {
                    "text": text,
                    "segments": segments,
...
def gemini_vision(prompt: str, image_base64: str, mime_type: str = "image/jpeg") -> str:
    """Analyze an image using Gemini 2.0 Flash. If GEMINI_API_KEY is missing or network fails,
    return a compact, fact-only placeholder JSON string to keep the pipeline running."""
    ...
    try:
        ...
        try:
            print(f"[warning] Gemini vision call failed: {e}")
        except Exception:
            pass
        return json.dumps(
            {
...
def build_story_groq(
    scenes: List[dict], transcript: dict, vision_results: List[dict]
) -> str:
    ...
    try:
        ...
        if isinstance(highlights, list):
            return [
                {
                    "id": i + 1,
                    "start_time": scenes[i % len(scenes)].get("start_time", i * 10)
                    if scenes
                    else i * 10,
                    "end_time": scenes[i % len(scenes)].get("end_time", (i + 1) * 10)
                    if scenes
                    else (i + 1) * 10,
    ...
    return [
        {
            ...
            "end_time": (i + 1) * 20,
            "title": f"Scene {i + 1}",
            "description": f"Key moment in scene {i + 1}",
            "category": "meeting",
            "importance": 0.8,
        }
...
def generate_memes(story: str, transcript_text: str) -> List[dict]:
            ...
            return [
                {
                    "id": i + 1,
                    "caption": m["caption"],
                    "timestamp": m.get("timestamp", i * 20),
                }
    ...
    return [
        {
            "id": 1,
            "caption": "AI agents working harder than me on a Monday morning",
            "timestamp": 0,
        },
        {"id": 2, "caption": "When the demo works on the first try", "timestamp": 30},
        {
            "id": 3,
            "caption": "My brain processing 50K events per second (it's not)",
            "timestamp": 60,
        },
        {"id": 4, "caption": "QA engineer has entered the chat", "timestamp": 90},
        {
            "id": 5,
            "caption": "That meeting that actually accomplished something",
            "timestamp": 120,
        },
    ]
...
def run_pipeline(job_id: str, video_path: str, jobs_store: Dict[str, Any]):
    ...
    try:
        ...
        # Allow configurable processing root to avoid permission issues on /data
        processing_root = os.environ.get("PROCESSING_DIR", "/data/processing")
